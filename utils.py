...
def pull_ollama_model(model_name):
    json_response = requests.post(url = f"http://127.0.0.1:11434/api/pull", json = {"model": model_name}).json()
    print(json_response)
...
def list_ollama_models():
    json_response = requests.get(url = "http://127.0.0.1:11434/api/tags").json()
    if json_response.get("error", False):
        return []
